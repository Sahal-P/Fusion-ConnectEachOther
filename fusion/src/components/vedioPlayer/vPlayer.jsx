...
export const VPlayer = () => {
    ...
    const TogglePlayPause = () => {
        vedioRef.current.paused ? vedioRef.current.play() : vedioRef.current.pause()
    }
    const playEvent = () => {
    ...
    const HandleKeyBoardControls = (event) => {
        ...
        switch (event.key.toLocaleLowerCase()) {
            case "k": TogglePlayPause()
                break
            case "f": HandleFullScreen()
                break
            case "i": HandleMiniPlayer()
                break
            case "t": HandleTheaterMode()
                break
            case "m": HandleMuteBtn()
                break
            case "arrowleft":
            case "j": skip(-5)
                break
    ...
    const HandleTheaterMode = () => { // vedioRef.current.requestFullscreen()
    ...
    const HandleTimeUpdate = () => {
        currentTime.current.textContent = formatDuration(vedioRef.current.currentTime)
    }
    const skip = (duration) => {
    ...
    return (
        <div className="vP-vedio-component-base">
            ...
            <div onKeyDown={HandleKeyBoardControls}
                    ...
                    <div ref={timelineCont} className="vP-video-timeline-container">
                        ...
                        <div className="vP-video-controls">
                            ...
                            <div className="vP-duration-container">
                                <div ref={currentTime}
                                    className="vP-current-time">0:00</div>
                                /
                                <div ref={totalTime}
                                    className="vP-total-time"></div>
                            </div>

                            <button className="vP-captions-btn"
                            ...
                            <button ref={speedBtn} onClick={HandlePlayBackSpeed} className="vP-speed-btn vP-wide">
                                1x
                            </button>

                            <button onClick={HandleMiniPlayer}
                                className="vP-mini-player-btn">
                                <svg viewBox="0 0 24 24">
                                    <path fill="currentColor" d="M21 3H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H3V5h18v14zm-10-7h9v6h-9z"/>
                                </svg>
                            </button>
                            <button onClick={HandleTheaterMode}
                                className="vP-theater-btn">
